...
with open(sys.argv[1], 'r') as infile, open(sys.argv[2], 'w') as outfile:
    ...
    for n, row in enumerate(dr):
        ...
        # convert Y/N values to "Yes" and "No"
        for col in ['subscription_req', 'full_text_search']:
            if row[col] == 'Y':
                row[col] = 'Yes'
            elif row[col] == 'N':
                row[col] = 'No'
            else:
                print('Line {0} Bad data, "{1}", found in Y/N boolean \
                    column.'.format(n, row[col]))

        # create display ranges for dates available and dates published
        row['range_avail'] = '{0}-{1}'.format(avail_start, avail_end)
        row['range_pub'] = '{0}-{1}'.format(row['year_pub_start'], 
            row['year_pub_end'])

        # generate a field holding the range of years between the begin and end years
        print(row['id'], avail_start, avail_end) 
